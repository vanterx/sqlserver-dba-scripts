    ...
    Process
    {
        foreach($SQLInstance in $ServerInstance)
        {
            Write-Verbose "Querying ServerInstance '$SQLInstance'"
                ...
                {
                    $ConnectionString = "Server={0};Database={1};Integrated Security=False;Connect Timeout={2};Encrypt={3};User ID={4};Password={5};" -f $SQLInstance,$Database,$ConnectionTimeout,$Encrypt,$Login,$Password
                }
                else
                {
