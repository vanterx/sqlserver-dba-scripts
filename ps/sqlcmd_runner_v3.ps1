...
#{
    ...
    .PARAMETER Login
        If specified, use a SQL login.

    .PARAMETER Password
        If specified, use a SQL password
    ...
    Process
    {
        foreach($SQLInstance in $ServerInstance)
        {
            Write-Verbose "Querying ServerInstance '$SQLInstance'"
                ...
                {
                    $ConnectionString = "Provider=MSOLEDBSQL;Data Source={0};Initial Catalog={1};Authentication=SqlPassword;Connect Timeout={2};Use Encryption for Data={3};User ID={4};Password={5};" -f $SQLInstance,$Database,$ConnectionTimeout,$Encrypt,$Login,$Password
                }
                elseif ($AuthType -eq "w")
                {
                    $ConnectionString = "Provider=MSOLEDBSQL;Data Source={0};Initial Catalog={1};Integrated Security=SSPI;Connect Timeout={2};Use Encryption for Data={3}" -f $SQLInstance,$Database,$ConnectionTimeout,$Encrypt

                }
                elseif ($AuthType -eq "amf")
                {
                    $ConnectionString = "Provider=MSOLEDBSQL;Data Source={0};Initial Catalog={1};Authentication=ActiveDirectoryInteractive;Connect Timeout={2};Use Encryption for Data={3};User ID={4};" -f $SQLInstance,$Database,$ConnectionTimeout,$Encrypt,$Login
                }
                elseif ($AuthType -eq "adi")
                {
                    $ConnectionString = "Provider=MSOLEDBSQL;Data Source={0};Initial Catalog={1};Authentication=ActiveDirectoryIntegrated;Connect Timeout={2};Use Encryption for Data={3};" -f $SQLInstance,$Database,$ConnectionTimeout,$Encrypt
                }
                elseif ($AuthType -eq "adp")
                {
                    $ConnectionString = "Provider=MSOLEDBSQL;Data Source={0};Initial Catalog={1};Authentication=ActiveDirectoryPassword;Connect Timeout={2};Use Encryption for Data={3};User ID={4};Password={5};" -f $SQLInstance,$Database,$ConnectionTimeout,$Encrypt,$Login,$Password
                }
                elseif ($AuthType -eq "ads")
                {
                   $ConnectionString = "Provider=MSOLEDBSQL;Data Source={0};Initial Catalog={1};Authentication=ActiveDirectoryServicePrincipal;Connect Timeout={2};Use Encryption for Data={3};User ID={4};Password={5};" -f $SQLInstance,$Database,$ConnectionTimeout,$Encrypt,$Login,$Password
                }
                else #unknown
                {
			Write-Host "unknown authentication method"
			Exit
                }
		...
		$conn = New-Object System.Data.OleDb.OleDbConnection
            ...
            Catch [SqlException] # For SQL exception
            {
                $Err = $_
